...
def customer_data(invoice, sales_invoice_doc):
    ...
    try:
        ...
        if customer_doc.custom_b2c != 1:
            ...
            if not address:
                frappe.throw(f"{customer_doc} Customer address is mandatory for non-B2C customers.")
...
def add_document_level_discount_with_tax(invoice, sales_invoice_doc):
    ...
    try:
        ...
        ET.SubElement(cac_allowance_charge, "cbc:ChargeIndicator").text = "false"

        ET.SubElement(
            cac_allowance_charge, "cbc:AllowanceChargeReasonCode"
        ).text = str(sales_invoice_doc.custom_zatca_discount_reason_code or "")

        ET.SubElement(
            cac_allowance_charge, "cbc:AllowanceChargeReason"
        ).text = str(sales_invoice_doc.custom_zatca_discount_reason or "")

        # Determine discount amount
        currency = str(sales_invoice_doc.currency).upper()
        ...
        discount_value = base_discount_amount if currency == "SAR" else discount_amount

        ET.SubElement(
            cac_allowance_charge, "cbc:Amount", currencyID=currency
        ).text = f"{discount_value:.2f}"
        ...
        tax_category_code = tax_category_map.get(sales_invoice_doc.custom_zatca_tax_category, "O")

        cac_tax_category = ET.SubElement(cac_allowance_charge, "cac:TaxCategory")
        ET.SubElement(cac_tax_category, "cbc:ID").text = tax_category_code

        # Ensure sales_invoice_doc.taxes exists and contains rates
        tax_rate = "0.00"  # Default tax rate
        if sales_invoice_doc.taxes and hasattr(sales_invoice_doc.taxes[0], "rate"):
            tax_rate = f"{float(sales_invoice_doc.taxes[0].rate):.2f}"

        ET.SubElement(cac_tax_category, "cbc:Percent").text = tax_rate

        cac_tax_scheme = ET.SubElement(cac_tax_category, "cac:TaxScheme")
        ET.SubElement(cac_tax_scheme, "cbc:ID").text = "VAT"

        return invoice

    except (ET.ParseError, AttributeError, ValueError) as e:
        frappe.log_error(f"Error processing allowance charge: {str(e)}")
        frappe.throw("An error occurred while processing the discount data.")
        return None
